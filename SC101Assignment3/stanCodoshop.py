...
def get_best_pixel(pixels):
    ...
    lowest_pixel_dist = float('inf')
    best = ''
    for pixel in pixels:
        pixel_dist = get_pixel_dist(pixel, avg_red, avg_green, avg_blue)
        if pixel_dist < lowest_pixel_dist:
            best = pixel
            lowest_pixel_dist = pixel_dist
    return best
...
def solve(images):
    ...
    # for loop each (x, y) to find out the best pixel.
    for x in range(width):
        for y in range(height):
            pixels = []

            # from input images (List) to find out all pixels of the same (x, y) to put into a list.
            for image in images:
                pixel = image.get_pixel(x, y)
                pixels.append(pixel)

            # find the best pixel of (x, y) and put on the black img.
