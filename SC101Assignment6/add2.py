...
Name: Astrid Chen
------------------------
This program will add two linked lists, each representing a positive integer.
The digits in each linked list are in reverse order.
The program will finally return a new linked list as the result.
"""
...
def add_2_numbers(l1: ListNode, l2: ListNode) -> ListNode:
    #######################
    total = convert_to_num(l1) + convert_to_num(l2)
    head = None
...
def convert_to_num(head: ListNode) -> int:
    lst, cur = [], head
    while cur:
        lst.append(cur.val)
        cur = cur.next
